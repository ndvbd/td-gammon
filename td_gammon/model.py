...
class TDGammonNew(TDGammon):
    def __init__(self, hidden_units, num_residuals, lr, lamda, init_weights, seed=123, input_units=198, output_units=1, save_path=None):
        super(TDGammonNew, self).__init__(hidden_units, lr, lamda, init_weights, seed=seed, input_units=198, output_units=1, save_path=save_path)

        self.hidden = nn.Linear(input_units, hidden_units)

        self.residuals = nn.ModuleList([Residual(hidden_units) for _ in range(num_residuals)])
